...
function TourOptionPreview({ t }) {
  return (
    <CustomerFrame
      url="/options/preview"
      hero={
        <div className="cust-hero" style={{backgroundImage:`linear-gradient(180deg, rgba(0,0,0,0.05) 0%, rgba(0,0,0,0.55) 100%), url("${window.__resources.heroTour}")`}}>
...
function ProductPagePreview({ page, links, lookup, selectedLinkId, selectLink, tourOption }) {
  ...
  return (
    <CustomerFrame
      ...
      badge="掲載プレビュー"
      hero={
        <div className="cust-hero" style={{backgroundImage:`linear-gradient(180deg, rgba(0,0,0,0.05) 0%, rgba(0,0,0,0.55) 100%), url("${window.__resources.heroPage}")`}}>
